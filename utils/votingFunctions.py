...
def makeAndCountVotes(voters, blockchain, NUM_OF_BLOCKS, VOTES_PER_BLOCK) -> tuple[int, int]:
    wallets = [v._wallet for v in voters]
...
def tallyVotes(blockchain, candidate_A_addr, candidate_B_addr):
    ...
    for block in blockchain.chain[1:]:
        for tx in block.transactions:
            if tx.receiver == candidate_A_addr.address:
                count_A += 1
            elif tx.receiver == candidate_B_addr.address:
                count_B += 1
